...
function buildLedgers(rows) {
  ...
  const baseLedgers = [
    { ledgerName: safeLedgerName($("salesLedgerName").value, "Amazon Sales"), group: "Sales Accounts" },
    { ledgerName: safeLedgerName($("salesReturnLedgerName").value, "Amazon Sales Return"), group: "Sales Accounts" },
    { ledgerName: safeLedgerName($("amazonLedgerName").value, "Amazon Marketplace"), group: $("ledgerGroup").value },
    { ledgerName: safeLedgerName($("tcsLedgerName").value, "Amazon TCS Receivable"), group: "Current Assets" },
    ...buildDefaultTaxLedgers()
  ];
  baseLedgers.forEach((ledger) => {
    if (!ledger.ledgerName) return;
    ledgerMap.set(ledger.ledgerName, { ...ledger, gstin: "", state: "NA", openingBalance: 0 });
  });
  rows.forEach((row) => {
    const name = buildLedgerName(row);
    ...
    const dynamicSalesLedger = resolveSalesLedgerName(row);
    if (dynamicSalesLedger && !ledgerMap.has(dynamicSalesLedger)) {
      ledgerMap.set(dynamicSalesLedger, {
        ledgerName: dynamicSalesLedger,
...
function buildVouchers(rows) {
  const salesReturnLedger = safeLedgerName($("salesReturnLedgerName").value, "Amazon Sales Return");
  const tcsLedger = safeLedgerName($("tcsLedgerName").value, "Amazon TCS Receivable");
  const rawEntries = rows.flatMap((row) => {
    const partyLedger = buildLedgerName(row);
    const salesLedger = row.isReturn
      ? salesReturnLedger
      : resolveSalesLedgerName(row);
    const entries = [{
  ...
  return aggregateVouchers(rawEntries);
}
...
function buildTallyXml({ companyName, ledgers, vouchers }) {
  const ledgerSeed = [
    ...buildDefaultTaxLedgers(),
    ...ledgers,
    ...collectVoucherLedgers(vouchers)
  ];

  const cleanLedgers = dedupeBy(
    ledgerSeed
      .map((ledger) => sanitizeLedgerMaster(ledger))
      .filter((ledger) => ledger.ledgerName),
    (ledger) => ledger.ledgerName.toUpperCase()
  );

  const cleanVouchers = vouchers
    .map((voucher) => ({
      ...voucher,
      partyLedger: safeLedgerName(voucher.partyLedger, "Miscellaneous Sales"),
      salesLedger: safeLedgerName(voucher.salesLedger, "Amazon Sales")
    }))
    .filter((voucher) => voucher.partyLedger && voucher.salesLedger && voucher.voucherNumber);

  const ledgerXml = cleanLedgers.map((ledger) => `
    <TALLYMESSAGE xmlns:UDF="TallyUDF">
      <LEDGER NAME="${escapeXml(ledger.ledgerName)}" RESERVEDNAME="">
        ...
        ${ledger.gstRegistrationType ? `<GSTREGISTRATIONTYPE>${escapeXml(ledger.gstRegistrationType)}</GSTREGISTRATIONTYPE>` : ""}
        ${ledger.gstin ? `<PARTYGSTIN>${escapeXml(ledger.gstin)}</PARTYGSTIN>` : ""}
        ${ledger.gstin ? `<GSTIN>${escapeXml(ledger.gstin)}</GSTIN>` : ""}
        ${ledger.taxType ? `<TAXTYPE>${escapeXml(ledger.taxType)}</TAXTYPE>` : ""}
      </LEDGER>
    </TALLYMESSAGE>`).join("");

  const voucherXml = cleanVouchers.map((voucher) => `
    <TALLYMESSAGE xmlns:UDF="TallyUDF">
...
function safeLedgerName(value, fallback) {
...
function buildLedgerName(row) {
  const name = safeLedgerName(row.partyName, "");
  if (name) return name;
  if (row.gstin) return `Customer-${row.gstin}`;
  return "Miscellaneous Sales";
}

function buildPartyFallback(row) {
  if (row.gstin) return `Party ${row.gstin}`;
  if (row.orderId) return `Amazon Party ${row.orderId}`;
  if (row.invoiceNumber) return `Amazon Party ${row.invoiceNumber}`;
  return "Amazon Party";
}

function buildDefaultTaxLedgers() {
  return [
    { ledgerName: "Output CGST", group: "Duties & Taxes", taxType: "Central Tax" },
    { ledgerName: "Output SGST", group: "Duties & Taxes", taxType: "State Tax" },
    { ledgerName: "Output IGST", group: "Duties & Taxes", taxType: "Integrated Tax" }
  ];
}

function collectVoucherLedgers(vouchers) {
  const voucherLedgers = [];
  vouchers.forEach((voucher) => {
    const partyLedger = safeLedgerName(voucher.partyLedger, "");
    const salesLedger = safeLedgerName(voucher.salesLedger, "");
    if (partyLedger) {
      voucherLedgers.push({ ledgerName: partyLedger, group: $("ledgerGroup").value });
    }
    if (salesLedger) {
      voucherLedgers.push({ ledgerName: salesLedger, group: inferLedgerGroup(salesLedger) });
    }
    if (voucher.igst) voucherLedgers.push({ ledgerName: "Output IGST", group: "Duties & Taxes", taxType: "Integrated Tax" });
    if (voucher.cgst) voucherLedgers.push({ ledgerName: "Output CGST", group: "Duties & Taxes", taxType: "Central Tax" });
    if (voucher.sgst) voucherLedgers.push({ ledgerName: "Output SGST", group: "Duties & Taxes", taxType: "State Tax" });
  });
  return voucherLedgers;
}

function sanitizeLedgerMaster(ledger) {
  const ledgerName = safeLedgerName(ledger.ledgerName, "");
  const group = safeLedgerName(ledger.group, inferLedgerGroup(ledgerName));
  const gstin = sanitizeGstin(ledger.gstin || "");
  const normalized = {
    ...ledger,
    ledgerName,
    group,
    gstin,
    openingBalance: round2(ledger.openingBalance || 0)
  };
  if (gstin) normalized.gstRegistrationType = ledger.gstRegistrationType || "Regular";
  return normalized;
}

function inferLedgerGroup(ledgerName) {
  const normalized = String(ledgerName || "").toUpperCase();
  if (!normalized) return "Sundry Debtors";
  if (normalized === "OUTPUT IGST" || normalized === "OUTPUT CGST" || normalized === "OUTPUT SGST") {
    return "Duties & Taxes";
  }
  if (normalized.includes("TCS")) return "Current Assets";
  if (normalized.includes("SALE")) return "Sales Accounts";
  return "Sundry Debtors";
}

function aggregateVouchers(vouchers) {
  ...
  vouchers.forEach((voucher) => {
    ...
    current.taxableValue = round2(current.taxableValue + voucher.taxableValue);
    current.igst = round2(current.igst + voucher.igst);
    current.cgst = round2(current.cgst + voucher.cgst);
    current.sgst = round2(current.sgst + voucher.sgst);
    current.totalAmount = round2(current.totalAmount + voucher.totalAmount);
  });
  return [...grouped.values()];
}

function resolveSalesLedgerName(row) {
  const baseSalesLedger = safeLedgerName($("salesLedgerName").value, "Amazon Sales");
  const mode = getPostingModeForRow(row);
  if (mode === "summary") return baseSalesLedger;
  if (mode === "hsn") {
    const hsn = safeLedgerName(row.hsn, "NA");
    return `${baseSalesLedger} - HSN ${hsn}`;
  }
  const itemName = compactItemName(row.itemDescription);
  return `${baseSalesLedger} - ${itemName}`;
}

function getPostingModeForRow(row) {
  if (row.sourceType === "B2B") return $("b2bPostingMode")?.value || "summary";
  if (row.sourceType === "B2C") return $("b2cPostingMode")?.value || "summary";
  return "summary";
}

function compactItemName(value) {
  const cleaned = safeLedgerName(value, "Item")
    .replace(/\s+/g, " ")
    .trim();
