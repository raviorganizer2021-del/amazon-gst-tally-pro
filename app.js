...
function buildLedgers(rows) {
  ...
  const baseLedgers = [
    ...
    { ledgerName: safeLedgerName($("tcsLedgerName").value, "Amazon TCS Receivable"), group: "Duties & Taxes" },
  ...
  rows.forEach((row) => {
    const name = safeLedgerName(row.partyName, buildPartyFallback(row));
...
function buildVouchers(rows) {
  ...
  const rawEntries = rows.flatMap((row) => {
    const partyLedger = safeLedgerName(row.partyName, buildPartyFallback(row));
...
function buildTallyXml({ companyName, ledgers, vouchers }) {
  const cleanLedgers = dedupeBy(
    ledgers
      .map((ledger) => ({
        ...ledger,
        ledgerName: safeLedgerName(ledger.ledgerName, ""),
        group: safeLedgerName(ledger.group, "Sundry Debtors")
      }))
  ...
  const cleanVouchers = vouchers
    .map((voucher) => ({
      ...voucher,
      partyLedger: safeLedgerName(voucher.partyLedger, "Amazon Party"),
